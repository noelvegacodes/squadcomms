<script>
	...
	const { form, message, errors, constraints, enhance, submitting } = superForm(
...
<div class="max-w-2xl w-full mx-auto py-10">
			...
			<form method="POST" use:enhance class="border-2 p-10 rounded-lg">
				...
				<div class="flex items-center gap-4">
					<button
						disabled={$submitting}
						type="submit"
						class="text-white bg-slate-500 py-1 p-4 rounded-lg">Reset Password</button
					>
					{#if $submitting}
						<div role="status">
							<svg
	...
	{:else}
		<p in:fade={{ delay: 200 }} class="text-center">
			Check your email for a reset link. Expires in 5 minutes
		</p>
	{/if}
</div>
